            ...
            >
              <div className="relative aspect-square">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  ...
                  <button
                    onClick={() => !img.id.startsWith('pub-') && openEditModal(img)}
                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-white transition-colors ${img.id.startsWith('pub-') ? 'bg-white/10 cursor-not-allowed' : 'bg-white/20 hover:bg-white/40'}`}
                    title={img.id.startsWith('pub-') ? 'Built-in image' : 'Edit'}
                  >
                    <FaEdit size={12} />
                  </button>
                  <button
                    onClick={() => !img.id.startsWith('pub-') && setDeleteId(img.id)}
                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-white transition-colors ${img.id.startsWith('pub-') ? 'bg-slate-500/50 cursor-not-allowed' : 'bg-red-500/70 hover:bg-red-600'}`}
                    title={img.id.startsWith('pub-') ? 'Built-in image — cannot delete' : 'Delete'}
                  >
                    <FaTrash size={12} />
                  </button>
                </div>
